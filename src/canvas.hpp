...
class Canvas
{
	...
	Canvas(int32_t width, int32_t height):nCanvasWidth(width),nCanvasHeight(height) {
	    vSurface.reserve(nCanvasWidth * nCanvasHeight);
	}

	[[nodiscard]]int32_t GetCanvasWidth() const {
	    return nCanvasWidth;
	}

	[[nodiscard]]int32_t GetCanvasHeight() const{
	    return nCanvasHeight;
	}
